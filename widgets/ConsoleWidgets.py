...
class ConsoleWidget(Labeler, ttk.Frame):
	...
	def __init__(self,parent:ttk.Frame,labeltext:str='Console: ',entrylabeltext:str='Command: ',labelside=tk.TOP,buttontext:str='Run',is_child=False,**kwargs):
		...
		self.entry = LabeledButtonEntry(self,labeltext=entrylabeltext,buttontext=buttontext,command=self._on_command,**kwargs)
		self.entry.pack(fill='x', expand=False, side=tk.TOP)

	def _on_command(self, event=None):
		if self.command:self.command(self.entry.get().strip())
		self.entry.clear()
